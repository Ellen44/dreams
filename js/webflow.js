...
Webflow.define('ix', function ($, _) {
  ...
  function init(list) {
    ...
    _.each(list, function (item) {
      ...
      // When a touch device meets a non-touch trigger, bypass styles.
      if (touch && _.any(item.value.triggers, nonTouch)) setStyles = false;
    });
  ...
  function build(i, el) {
    ...
    var triggers = config[id].triggers;
    if (!triggers) return;
    var state = store[id] || (store[id] = {});

    // Set initial styles using tram to help fallback-mode browsers
    var styles = config[id].style;
    if (setStyles && styles) tram(el).set(styles);
